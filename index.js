...
function findMinMax([firstElement, ...restElements]) {
  ...
  for (const index_restElements in restElements) {
    minValue =
      minValue > restElements[index_restElements]
        ? restElements[index_restElements]
        : minValue;
    maxValue =
      maxValue < restElements[index_restElements]
        ? restElements[index_restElements]
        : maxValue;
  }
...
function swapMinMax(array) {
  ...
  for (const index_Array in array) {
    minIndex =
      array[minIndex] > array[index_Array] ? minIndex == index_Array : minIndex;
    // console.log(minIndex);
    maxIndex =
      array[maxIndex] < array[index_Array] ? array[index_Array] : maxIndex;
...
function getEvenNumbers(arr) {
  ...
  for (const arrItem of arr) {
    if (arrItem % 2 === 0) {
      result[itemResult] = arrItem;
      // console.log(result[itemResult]); // для теста себя - просто проверка
      itemResult++;
    }
  }
  return result;
...
function multiplyByIndex(arr) {
  ...
  for (const indexItem in arr) {
    result[itemResult] = arr[indexItem] * indexItem;
    itemResult++;
  }
  return result;
}
...
function replaceNumbers(arr) {
  ...
  for (const arrItem of arr) {
    if (arrItem > 10) {
      result[itemResult] = "Greater than 10";
    } else {
      result[itemResult] = "Less than or equal to 10";
    }
    itemResult++;
  }
  return console.log(result, result.length); // добавил - result.length : как то подозрительно выводится
  // наверное из-за пятница, 13-е....
}
